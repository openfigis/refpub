...
public class RefPubImplementation implements RefPubInterface {
	...
	@Override
	///////////////////////////
	public Concept getAttributeForObject(String concept, String codesystem, String code, String attribute) {
	...
	@Override
	public AttributeList getAllAttributesForConceptAndCodesystem(String concept, String codesystem) {
	...
	@Override
	public ConceptList getSubGroups(String Concept, String filter, String group, String subGroup) {
	...
	private List<RefPubConcept> getAllConcepts(String count, String page) {
		...
		if (page == null) {
			page = Integer.toString(Integer.parseInt(min + 1));
		}

		List<RefPubConcept> concepts = new ArrayList<RefPubConcept>();

		List<MDConcept> listOfConcepts = ps.getConcepts(RefPubImplementation.CONFIGURATION.getDb_schema()); // Retrieves
																											// the
																											// concepts
																											// from the
																											// persistence
																											// layer
		for (MDConcept c : listOfConcepts) { // for each concept try to set-up the codelist to be pushed into
												// RefPubObjects
	...
	private List<RefPubObject> getAllObjectsForConceptFlat(String concept, String count, String page) {
		...
		String nameColumn = Utils.guessNameColumn(
				ps.getTableColumns(RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name()));

		List<RefPubObject> returnList = Utils
				.buildRefPubObjectList(ps.getObjectsFlat(RefPubImplementation.CONFIGURATION.getDb_schema(),
						tbl.getName(), tbl.getPrimaryKey(), nameColumn, min, max));
	...
	private List<RefPubObject> getAllObjectsForConcept(String concept, String count, String page) {
		...
		ArrayList<HashMap<String, Object>> objs = ps.getObjects(RefPubImplementation.CONFIGURATION.getDb_schema(), meta,
				mdconcept.getMeta_column(), cp.getTable_name(), tbl.getPrimaryKey(), min, max);
		if (objs == null) {
	...
	private RefPubObject getSingleObject(String concept, String codelist, String code) {
		MDConcept mdconcept = ps.getConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept);
		MDCodelist mdcodelist = ps.getCodeList(RefPubImplementation.CONFIGURATION.getDb_schema(), concept, codelist);

		TableReference tbl = ps.getTableReferenceByName(RefPubImplementation.CONFIGURATION.getDb_schema(),
				mdconcept.getTable_name());

		RefPubObject refPubObject = Utils
				.buildRefPubObject(ps.getObject(RefPubImplementation.CONFIGURATION.getDb_schema(),
						mdconcept.getTable_name(), mdcodelist.getCode_column(), code, tbl.getPrimaryKey()));

		if (refPubObject == null) {
			return new RefPubObject();
		}

		List<CodeListDAO> codemap = Utils.retrieveCodeListForObject(
				ps.getCodelistForConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept), refPubObject);

		if (refPubObject.getMETA() != null) {
			HashMap<String, Object> mdRefObj = ps.getMDRefObject(RefPubImplementation.CONFIGURATION.getDb_schema(),
	...
	private RefPubObject getSingleObject(String concept, String code) {
		...
		List<CodeListDAO> codemap = Utils.retrieveCodeListForObject(
				ps.getCodelistForConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept), obj);
	...
	private RefPubObject getSingleAttributeForSingleObject(String concept, String codelist, String code,
			String attribute) {
		MDConcept mdconcept = ps.getConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept);
		...
		String targetColumn = mdcodelistSource.getCode_column();
		if (mdcodelistTarget != null) {
			targetColumn = mdcodelistTarget.getCode_column();
		}

		TableReference tbl = ps.getTableReferenceByName(RefPubImplementation.CONFIGURATION.getDb_schema(),
				mdconcept.getTable_name());

		RefPubObject obj = Utils.buildRefPubObject(ps.getAttributeForSingleObject(
				RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name(),
				mdcodelistSource.getCode_column(), targetColumn, code, tbl.getPrimaryKey(), attribute));
	...
	private List<RefPubObject> getObjectsByCodeList(String concept, String codelist, String count, String page) {
		...
		// add a condition, not all ref objects do have a formal metadata id
		if (con.getMeta_column() == null || con.getMeta_id() == null) {
			l = ps.getObjectsByCodeListWithoutMeta(RefPubImplementation.CONFIGURATION.getDb_schema(),
					cl.getTable_name(), cl.getCode_column(), ti.getPrimaryKey(), min, max);
		} else {
			l = ps.getObjectsByCodeList(RefPubImplementation.CONFIGURATION.getDb_schema(), cl.getTable_name(),
					con.getMeta_column(), con.getMeta_id(), cl.getCode_column(), ti.getPrimaryKey(), min, max);
		}

		List<RefPubObject> returnList = Utils.buildRefPubObjectList(l);
	...
	private List<RefPubObject> getGroupsByFilter(String concept, String filter) {
		...
		if (mdGrouping == null) {
			mdGrouping = ps.getGroupByFilter(RefPubImplementation.CONFIGURATION.getDb_schema(), filter);
		}
		...
		List<RefPubObject> topHierarchy = Utils
				.buildRefPubObjectList(ps.getGroupsValues(RefPubImplementation.CONFIGURATION.getDb_schema(), hl));
	...
	private List<RefPubObject> getSubGroup(String concept, String filter, String group) {
		...
		/*
		 * if ("commodities".equals(concept.toLowerCase())) { return this.getSubGroupAlternative(concept, filter,
		 * group); }
		 */
		...
		fullList = Utils.buildRefPubObjectList(ps.getRootHierarchy(dbSchema, mdGrouping.getHierarchy_item_table(),
				hItemTable.getPrimaryKey(), hItemTable.getMetaColumn(), totalDepth.get(0)));

		/* Getting SubGroups */
		if (totalDepth.size() < 1) {
			totalDepth.add(mdGrouping.getMeta());
		}
		List<RefPubObject> subGroupsList = Utils.buildRefPubObjectList(ps.getSubGroupHierarchy(dbSchema,
				itemTable.getName(), hItemTable.getName(), hGroupTable.getName(), itemTable.getPrimaryKey(),
				hItemTable.getPrimaryKey(), itemTable.getMetaColumn(), hItemTable.getMetaColumn(),
				hGroupTable.getMemberColumn(), hGroupTable.getGroupColumn(), totalDepth));
		...
		List<RefPubObject> listObjects = Utils.buildRefPubObjectList(ps.getFilteredObjectsHierarchy(dbSchema,
				itemTable.getName(), hItemTable.getName(), hGroupTable.getName(), groupTable.getName(),
				itemTable.getPrimaryKey(), itemTable.getMetaColumn(), hItemTable.getPrimaryKey(),
				hItemTable.getMetaColumn(), hGroupTable.getMemberColumn(), hGroupTable.getGroupColumn(),
				groupTable.getGroupColumn(), groupTable.getMemberColumn(), hGroupTable.getMetaColumn(),
				mdGrouping.getFilter_meta(), totalDepth), true);
		...
		return Utils.cleanUpGroupList(
				Utils.setUpUpGroupList(Utils.setUpURIOnTree(Utils.setUpCodeListOnTree(fullList, mdCodelist, concept),
						this.BuildURI(null, null)), mdGrouping.getMeta()),
				mdGrouping.getMeta());
	}
	...
	private List<RefPubObject> getSubGroupSystemHierarchical(String concept, String filter, String group) {
		...
		List<RefPubObject> fullList = Utils.buildRefPubObjectHierarchyList(ps.getSubGroupHierarchyAlternative(dbSchema,
				itemTable.getName(), groupTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
				groupTable.getMemberColumn(), groupTable.getGroupColumn(), mainHierarchy.getFILTER()));
		...
		if (leafs.size() > 0) {
			...
			List<RefPubObject> childs = Utils.buildRefPubObjectList(ps.getChildsHierarchyAlternative(dbSchema,
					itemTable.getName(), groupTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
					groupTable.getMemberColumn(), groupTable.getGroupColumn(), mdGrouping.getMeta(), leafsPKID));
			for (RefPubObject child : childs) {
				fullList = Utils.attachObjectToRefPubTree(fullList, child);
			}
		}
		return Utils.setUpURIOnTree(Utils.setUpCodeListOnTree(fullList, mdCodelist, concept),
				this.BuildURI(null, null));
	}
	...
	private List<RefPubObject> getSubGroupFlatNoMeta(String concept, String filter, String group) {
		...
		if (!Utils.ALPHABETICAL_ID.equalsIgnoreCase(group)) {
			...
			List<RefPubObject> hierarchy = Utils.buildRefPubObjectHierarchyList(ps.getFlatHierarchyType(dbSchema,
					itemTable.getName(), groupTable.getName(), itemTable.getPrimaryKey(), groupTable.getMemberColumn(),
					groupTable.getGroupColumn(), group, totDepth.get(0)));
			return Utils.setUpURIOnTree(Utils.setUpCodeListOnTree(hierarchy, mdCodelist, concept),
					this.BuildURI(null, null));
		} else {
			...
			List<RefPubObject> hierarchy = Utils.buildRefPubObjectHierarchyList(ps.getFlatHierarchyTypeExtended(
					dbSchema, itemTable.getName(), groupTable.getName(), itemTable.getPrimaryKey(),
					groupTable.getMemberColumn(), groupTable.getGroupColumn(), metaList, totDepth.get(0)));
			return Utils.buildAlphabeticalOnTree(
					Utils.setUpURIOnTree(Utils.setUpCodeListOnTree(hierarchy, mdCodelist, concept),
							this.BuildURI(null, null)),
					"name_e", true);
		}
	}
	...
	private List<RefPubObject> getAlphabeticalGroup(String concept, String filter, String group) {
		...
		if (mdGrouping.getFilter_meta() != null) {
			list = Utils.buildRefPubObjectList(ps.getAplhabeticalByFilter(dbSchema, itemTable.getName(),
					groupTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
					groupTable.getMemberColumn(), groupTable.getGroupColumn(), mdGrouping.getFilter_meta()));
		}
	...
	private List<RefPubObject> getFlatGroupHierarchy(String concept, String filter, String group) {
		...
		retStrut = Utils.buildRefPubObjectList(ps.getFlatHierarchy(dbSchema, itemTable.getName(), groupTable.getName(),
				itemTable.getPrimaryKey(), itemTable.getMetaColumn(), groupTable.getMemberColumn(),
				groupTable.getGroupColumn(), mdGrouping.getFilter()));

		if (retStrut.size() < 1) {
			retStrut = Utils.buildRefPubObjectList(ps.getFlatHierarchy(dbSchema, itemTable.getName(),
					groupTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
					groupTable.getMemberColumn(), groupTable.getGroupColumn(), mdGrouping.getFilter_meta()));
		}

		return Utils.setUpCodeListOnTree(Utils.setUpURIOnTree(retStrut, this.BuildURI(null, null)), mdCodelist,
				concept);
	}
	...
	private List<RefPubObject> getAllHierarchy(String concept, String filter) {
		...
		List<RefPubObject> treeRes = Utils
				.buildRefPubObjectHierarchyList(ps.getEntireHierarchy(dbSchema, mdGrouping.getFilter()));
	...
	private List<RefPubObject> getFilterMetaGrouping(String concept, String filter, List<String> filterMeta) {
		...
		List<RefPubObject> retList = Utils.buildRefPubObjectList(ps.getMetaGrouping(dbSchema, itemTable.getName(),
				itemTable.getPrimaryKey(), itemTable.getMetaColumn(), filterMeta));
	...
	private List<RefPubObject> getParents(RefPubObject refPubObject, MDConcept mdconcept, TableReference tbl) {
		if (mdconcept.getTable_group() != null) {
			List<MDCodelist> codelist = ps.getCodelistForConcept(RefPubImplementation.CONFIGURATION.getDb_schema(),
					mdconcept.getRest_concept());
			List<RefPubObject> parents = Utils.buildRefPubObjectList(
					ps.getParentHierarchy(RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name(),
							mdconcept.getTable_group(), mdconcept.getTable_group_member(), mdconcept.getMeta_column(),
							refPubObject.getPKID(), mdconcept.getTable_group_column(), tbl.getPrimaryKey()));
			...
			if (parents.size() > 0) {
				for (int x = 0; x < parents.size(); x++) {
					...
					parents.get(x).setGroup_name(
							this.getValueFromAttributes(parents.get(x).getATTRIBUTES(), "MKP_GROUP_NAME"));
					parents.get(x)
							.setNAME(this.getValueFromAttributes(parents.get(x).getATTRIBUTES(), "SCIENTIFIC_NAME"));
					parents.get(x).setCodeList(Utils.retrieveCodeListForObject(codelist, parents.get(x)));
				}
			} else {
				String columnName = Utils.guessNameColumn(ps
						.getTableColumns(RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name()));
				parents = Utils.buildRefPubObjectList(ps.getRootParentHierarchy(
						RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name(),
						mdconcept.getTable_group(), mdconcept.getTable_group_member(), mdconcept.getMeta_column(),
						refPubObject.getPKID(), mdconcept.getTable_group_column(), columnName, tbl.getPrimaryKey()));
				for (int x = 0; x < parents.size(); x++) {
					parents.get(x).setCurrentURI(this.BuildURI("1", "1"));
					parents.get(x).setConcept(refPubObject.getConcept());
					parents.get(x).setGroup_name(
	...
	private List<RefPubObject> getChildren(RefPubObject refPubObject, MDConcept mdconcept, TableReference tbl) {
		if (mdconcept.getTable_group() != null) {
			...
			List<RefPubObject> children = Utils.buildRefPubObjectList(ps.getChildrenHierarchy(
					RefPubImplementation.CONFIGURATION.getDb_schema(), mdconcept.getTable_name(),
					mdconcept.getTable_group(), mdconcept.getTable_group_member(), mdconcept.getMeta_column(),
					refPubObject.getPKID(), mdconcept.getTable_group_column(), tbl.getPrimaryKey()));
			...
			for (int x = 0; x < children.size(); x++) {
				...
				children.get(x)
						.setGroup_name(this.getValueFromAttributes(children.get(x).getATTRIBUTES(), "MKP_GROUP_NAME"));
				children.get(x)
						.setNAME(this.getValueFromAttributes(children.get(x).getATTRIBUTES(), "SCIENTIFIC_NAME"));
			...
			/*
			 * if (children.size() > 0) { int counter = 0; for (RefPubObject c : children) { List<RefPubObject> itr =
			 * this.getChildren(c, mdconcept, tbl); if (itr.size() > 0) { c.setChildrens(this.getChildren(c, mdconcept,
			 * tbl)); c.setIs_group(true); children.set(counter, c); } counter++; } }
			 */
	...
	private List<GenericType> attributeListByConcept(String concept) {
		MDConcept cmt = ps.getConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept);
		List<GenericType> list = ps.getTableColumns(RefPubImplementation.CONFIGURATION.getDb_schema(),
				cmt.getTable_name());
	...
	private List<RefPubObject> attachObjectsToLeafs(List<RefPubObject> tree, String concept, String filter) {
		for (int i = 0; i < tree.size(); i++) {
			if (Utils.hasChildObject(tree.get(i))) {
				...
				MDCodelist mdCodelist = ps
						.getDefaultCodelistFromConcept(RefPubImplementation.CONFIGURATION.getDb_schema(), concept);
				MDGrouping mdGrouping = ps.getGroupByDescription(RefPubImplementation.CONFIGURATION.getDb_schema(),
						filter);

				TableReference itemTable = ps.getTableReferenceByName(RefPubImplementation.CONFIGURATION.getDb_schema(),
						mdGrouping.getItem_table());

				int countSubObjects = ps.getObjectByMetaCount(RefPubImplementation.CONFIGURATION.getDb_schema(),
						itemTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
						tree.get(i).getPKID());
				List<RefPubObject> newLeafs = new ArrayList<RefPubObject>();
				if (countSubObjects < 100) {
					newLeafs = Utils.buildRefPubObjectList(
							ps.getObjectByMeta(RefPubImplementation.CONFIGURATION.getDb_schema(), itemTable.getName(),
									itemTable.getPrimaryKey(), itemTable.getMetaColumn(), tree.get(i).getPKID()));
					...
					for (String offset : offsets) {
						...
						String numRows = "100";
						List<RefPubObject> newLeafsTemp = Utils.buildRefPubObjectList(
								ps.getObjectByMetaPaginate(RefPubImplementation.CONFIGURATION.getDb_schema(),
										itemTable.getName(), itemTable.getPrimaryKey(), itemTable.getMetaColumn(),
										tree.get(i).getPKID(), start, numRows));
